...
# Disable gi.overrides before importing any gi.repository modules
# to avoid unix_signal_add_full AssertionError in bundled PyGObject
...
# Try to import, catching the gi.overrides error if it still occurs
try:
    from gi.repository import GLib, Gst, Gio
except (AssertionError, AttributeError) as e:
    if 'unix_signal_add_full' in str(e):
        # gi.overrides is broken in this PyGObject version.
        # Remove it from sys.modules and try again with introspection only
        sys.modules.pop('gi.overrides', None)
        sys.modules.pop('gi.overrides.GLib', None)
        sys.modules.pop('gi.overrides.Gst', None)
        sys.modules.pop('gi.overrides.Gio', None)
        from gi.repository import GLib, Gst, Gio
    else:
        raise
